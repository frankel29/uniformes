        ...
        <div class="border border-gray-300 dark:border-gray-700">
            <div class="relative bg-gray-200">
                <a href="{{ url('products', $product->slug) }}" class="">
                    @if (!empty($product->images) && isset($product->images[0]))
                        ...
                        <p>{{ $imageUrl }}</p> <!-- Línea de depuración para ver la URL de la imagen -->
                    @else
            ...
            <div class="flex justify-center p-4 border-t border-gray-300 dark:border-gray-700">
                <a wire:click.prevent='addToCart({{ $product->id }})' href="#" class="text-gray-500 flex items-center space-x-2 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-300">
                    ...
                    </svg><span wire:loading.remove wire:target='addToCart({{ $product->id }})'>Añadir al carrito</span><span wire:loading 
                    wire:target='addToCart({{ $product->id }})'>Añadiendo...</span> -->
                </a>
            </div>
        </div>
    </div>
